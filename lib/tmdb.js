...
const { filterMoviesByAvailability, filterTVByAvailability } = require('./realdebrid')
...
const MOVIE_SORT_MAP = {
    ...
    'Drama': 'popularity.desc',
    'Comedy': 'popularity.desc',
    'Action': 'popularity.desc',
    'Thriller': 'popularity.desc',
    'Romance': 'popularity.desc',
    'Horror': 'popularity.desc'
}
...
const TV_GENRE_IDS = {
    ...
    'Romance': 10749,
    'Horror': 9648        // Mystery (no horror genre for TV, use mystery)
}
...
async function discoverMoviesPage(language, sortOption, page = 1) {
    ...
    const sortBy = MOVIE_SORT_MAP[sortOption] || MOVIE_SORT_MAP['Popular']
    ...
    if (GENRE_IDS[sortOption]) {
        params.with_genres = GENRE_IDS[sortOption]
    }
...
async function discoverTVPage(language, sortOption, page = 1) {
    const apiKey = await getApiKey()
    const sortBy = TV_SORT_MAP[sortOption] || TV_SORT_MAP['Popular']
    ...
    // Add genre filter if applicable
    if (TV_GENRE_IDS[sortOption]) {
        params.with_genres = TV_GENRE_IDS[sortOption]
    }
...
async function getCatalogContent(catalogId, type, sortOption, skip = 0) {
    ...
    if (sortOption === 'Popular' || GENRE_IDS[sortOption] || TV_GENRE_IDS[sortOption]) {
    ...
    // Fetch IMDB IDs (limit to top 50)
    const contentType = type === 'movie' ? 'movie' : 'tv'
    const cinemetaType = type === 'movie' ? 'movie' : 'series'
    items = await batchGetImdbIds(items.slice(0, 50), contentType)

    // Filter by Real Debrid availability
    if (type === 'movie') {
        items = await filterMoviesByAvailability(items)
    } else {
        items = await filterTVByAvailability(items)
    }
